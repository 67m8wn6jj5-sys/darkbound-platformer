...
EXPECTED_ATTACKS = {
    'attack_1': (TODAY_KO_ARCHIVE, TODAY_KO_ROOT, 'Ko Gasumi sword atta', 'The_warrior_shifts_his_weight_forward_tightening_h'),
    'attack_2': (TODAY_SWORD_ARCHIVE, TODAY_SWORD_ROOT, 'Sword attack', 'Upward_sword_slash._Starting_from_the_feet_and_fin'),
    'attack_3': (BASE_ARCHIVE, BASE_ROOT, 'Sword attack', 'The_character_raises_their_sword_in_a_swift_powerf'),
}


def fail(message):
    print(f'V27 PROTAGONIST VERIFY FAILED: {message}', file=sys.stderr)
    raise SystemExit(1)
...
def main():
    ...
    for action, counts in EXPECTED_COUNTS.items():
        ...
        if action.startswith('attack_'):
            archive, _root, state_name, anim_name = EXPECTED_ATTACKS[action]
            if meta.get('sourceArchive') != archive:
                fail(f'{action}: wrong source archive')
            if meta.get('sourceState') != state_name or meta.get('sourceAnimation') != anim_name:
                fail(f'{action}: wrong source animation mapping')
    ...
    if 'Upward_sword_slash' not in manifest['attack_2'].get('sourceAnimation',''):
        fail('second strike must remain the upward slash')
    if manifest['attack_3'].get('sourceAnimation') != 'The_character_raises_their_sword_in_a_swift_powerf':
        fail('third strike must be the restored overhead/downward finisher')
    if 'grounded and airborne downward finisher' not in manifest['attack_3'].get('gameplay',''):
        fail('downward finisher must be intentionally shared by grounded and airborne attack-3 presentation')
    ...
    selected_attacks = sum((source_attack_pngs(a) for a in sword_actions), [])
    output_pngs = sorted(OUT.rglob('*.png'))
    expected = kept_current + selected_attacks
    if len(kept_current) != 201:
        fail(f'expected 201 preserved non-attack/rotation PNGs, found {len(kept_current)}')
    if len(selected_attacks) != 48:
        fail(f'expected 48 frames across the three live strikes, found {len(selected_attacks)}')
    if len(output_pngs) != 249:
        fail(f'expected 249 production PNGs, found {len(output_pngs)}')
    if Counter(map(digest, expected)) != Counter(map(digest, output_pngs)):
        fail('production art is not the exact intended V27 composition')
    ...
    print('V27 protagonist artwork verification passed.')
    print('Live combo: today forward cut -> today upward cut -> restored current-archive downward finisher')
    print('All 48 live sword frames match their intended source animations byte-for-byte.')
    print('Production artwork: 249/249 PNGs match the intended composition.')
