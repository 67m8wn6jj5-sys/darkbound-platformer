...
assert.ok(EXPEDITION_V34.right-EXPEDITION_V34.left>=12000);
...
for(let depth=0;depth<4;depth++){
  ...
  assert.equal(room.checkpoints.length,8);
  assert.equal(room.lights.length,8);
  assert.equal(room.objects.length,3);
  assert.ok(room.worldWidth>=13000);
  assert.ok(room.groundSpawns.length>=18);
  assert.ok(room.perchSpawns.length>=10);
  assert.ok(room.platforms.length>=20);
  assert.ok(room.platforms.filter(spec=>spec.role==='upper').length>=5);
  for(let index=0;index<room.sections.length;index++){
    ...
    if(index>0)assert.equal(section.start,room.sections[index-1].end);
  }
  assert.ok(new Set(room.floorSegments.map(spec=>spec.y)).size>=3);
  let realGapCount=0;
  for(const section of room.sections){
    ...
    for(let index=1;index<floors.length;index++){
      ...
      if(gap>0){realGapCount++;assert.ok(gap<=128);}
    }
  }
  assert.ok(realGapCount>=2);
  assert.equal(room.collision.length,room.floorSegments.length+room.platforms.length);
  assert.ok(room.exitX>EXPEDITION_V34.right-400);
}
assert.equal(new Set(stageLabels).size,4);
...
assert.ok(GameSceneV34.prototype.isBranchDepth(0));assert.ok(GameSceneV34.prototype.isBranchDepth(2));assert.equal(GameSceneV34.prototype.isBranchDepth(1),false);
...
assert.match(source,/tilePositionX=scroll\*\.22/);
assert.match(source,/tilePositionX=scroll\*\.38/);
assert.match(source,/setScrollFactor\(0\)/);
assert.doesNotMatch(source,/ENVIRONMENT_ART_V30\.arches/);
assert.match(source,/renderGothicTerrain\(\[\.\.\.layout\.floorSegments,\.\.\.layout\.platforms\]\)/);
assert.match(source,/fallResetY/);assert.match(source,/updateCheckpointV34/);
assert.match(source,/AREA CLEARED • REACH THE GREEN GATE/);
