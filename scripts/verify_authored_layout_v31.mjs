...
for(const [id,authored] of Object.entries(AUTHORED_STAGE_V31)){
  ...
  const route=authored.platforms.filter(spec=>spec.role==='route').sort((a,b)=>a.x-b.x);assert.ok(route.length>=4);
  ...
  for(let index=1;index<route.length;index++){
    const previous=route[index-1],current=route[index];const gap=Math.max(0,current.x-(previous.x+previous.w));
    assert.ok(gap<=128,`${id} historical route gap drifted`);assert.ok(Math.abs(current.y-previous.y)<=64,`${id} historical rise drifted`);
  }
}
for(const template of ['duel','hunters','mixed','crossfire','pressure','barrage','elite']){
  for(let seed=1;seed<=20;seed++){
    ...
    assert.equal(room.floor.x,256);assert.equal(room.floor.y,640);assert.equal(room.floor.w,2304);assert.equal(room.floor.h,96);assert.equal(room.floorSegments.length,1);assert.equal(room.collision.length,room.platforms.length+1);
  }
}
...
const main=readFileSync('src/main.js','utf8');const v34=readFileSync('src/GameSceneV34.js','utf8');const v33=readFileSync('src/GameSceneV33.js','utf8');const v32=readFileSync('src/GameSceneV32.js','utf8');
assert.match(main,/import \{ GameSceneV34 \} from '\.\/GameSceneV34\.js'/);assert.match(main,/scene: \[GameSceneV34\]/);assert.match(v34,/extends GameSceneV33/);assert.match(v33,/extends GameSceneV32/);assert.match(v32,/extends GameSceneV31/);
console.log('Historical V31 authored-layout verification passed beneath V34.');
