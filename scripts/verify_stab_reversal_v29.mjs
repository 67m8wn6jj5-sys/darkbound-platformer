...
assert.equal(PIXELLAB_MANIFEST.attack_1.east,8);assert.equal(PIXELLAB_MANIFEST.attack_1.west,8);
assert.deepEqual(TUNING.attackDurationsMs,[230,245,500]);assert.deepEqual(TUNING.attackActiveStartMs,[52,60,80]);assert.deepEqual(TUNING.attackActiveEndMs,[144,164,220]);assert.deepEqual(TUNING.attackRanges,[80,88,104]);
const scene=Object.create(GameSceneV29.prototype);scene.attackStartsAt=1000;scene.comboStep=0;
...
assert.deepEqual(reversedFrames,forwardFrames.map(frame=>7-frame));assert.equal(reversedFrames[0],7);assert.equal(reversedFrames.at(-1),0);
assert.equal(reverseAttackFrameV29('attack_1','west',0),7);assert.equal(reverseAttackFrameV29('attack_1','west',7),0);assert.equal(reverseAttackFrameV29('attack_2','east',4),4);assert.equal(reverseAttackFrameV29('attack_3','east',5),5);
assert.deepEqual(reversedBladeTangentV29(4),{x:-8,y:-24});assert.deepEqual(reversedBladeTangentV29(3),{x:-14,y:-24});assert.deepEqual(reversedBladeTangentV29(2),{x:-18,y:-14});
...
assert.match(main,/import \{ GameSceneV36 \} from '\.\/GameSceneV36\.js'/);assert.match(main,/scene: \[GameSceneV36\]/);assert.match(main,/GameSceneV36 -> GameSceneV35 -> GameSceneV34 -> GameSceneV33 -> GameSceneV32 -> GameSceneV31 -> GameSceneV30 -> GameSceneV29 -> GameSceneV28/);
assert.match(v36,/extends GameSceneV35/);assert.match(v35,/extends GameSceneV34/);assert.match(v34,/extends GameSceneV33/);assert.match(v33,/extends GameSceneV32/);assert.match(v32,/extends GameSceneV31/);assert.match(v31,/extends GameSceneV30/);assert.match(v30,/extends GameSceneV29/);
console.log('V29 reversed stab verification passed beneath V36.');
