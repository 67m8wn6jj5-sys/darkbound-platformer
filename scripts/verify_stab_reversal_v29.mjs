...
assert.deepEqual(reversedFrames,forwardFrames.map(frame=>7-frame));
assert.equal(reversedFrames[0],7);assert.equal(reversedFrames.at(-1),0);
assert.equal(reverseAttackFrameV29('attack_1','west',0),7);
assert.equal(reverseAttackFrameV29('attack_1','west',7),0);
assert.equal(reverseAttackFrameV29('attack_2','east',4),4);
...
assert.match(source,/action!==['"]attack_1['"]/);
...
assert.match(main,/GameSceneV33 -> GameSceneV32 -> GameSceneV31 -> GameSceneV30 -> GameSceneV29 -> GameSceneV28/);
...
console.log('V29 reversed stab verification passed beneath V33.');
