...
assert.match(readFileSync('src/main.js','utf8'),/GameSceneV18/,'main must boot the V18 scene');
...
const s=Object.create(GameSceneV18.prototype);
s.comboStep=0;
s.attackPatternIndex=0;
assert.equal(s.attackVisualForStep(0,0),'attack_1');
assert.equal(s.attackVisualForStep(1,0),'attack_2');
assert.equal(s.attackVisualForStep(2,0),'attack_3');
assert.equal(s.attackVisualForStep(0,1),'attack_alt');
assert.equal(s.attackVisualForStep(1,1),'attack_2');
assert.equal(s.attackVisualForStep(0,2),'attack_1');
assert.equal(s.attackVisualForStep(1,2),'attack_alt');
assert.equal(s.attackVisualForStep(2,2),'attack_3');
...
s.attackStartsAt=1000;
s.comboStep=0;
const altFrames=[
  s.attackFrame('attack_alt','east',1000),
  s.attackFrame('attack_alt','east',1042),
  s.attackFrame('attack_alt','east',1080),
  s.attackFrame('attack_alt','east',1122),
  s.attackFrame('attack_alt','east',1185),
];
for(let i=1;i<altFrames.length;i++){
  assert.ok(altFrames[i]>=altFrames[i-1],`alternate swing frames must advance monotonically: ${altFrames}`);
}
assert.ok(altFrames[1]>=2&&altFrames[3]<=6,'alternate active frames must line up with attack-1 contact window');

s.comboStep=2;
const finisherEnd=s.attackFrame('attack_3','east',1430);
assert.equal(finisherEnd,7,'heavy finisher must reach its final recovery frame by the shortened end time');
