...
for(const [action,profile] of Object.entries(BLADE_TRACK_V27)){
  ...
  assert.ok(profile.moteLifeMs<=68);assert.ok(profile.moteDrift<=12);
  assert.ok(profile.moteFrames.length<=3);
...
assert.ok(bladeTangentV27('attack_1',4).x>0);
assert.ok(bladeTangentV27('attack_2',4).y<0);
assert.ok(bladeTangentV27('attack_3',5).y>0);
const source=readFileSync('src/GameSceneV27.js','utf8');
assert.match(source,/bladeWorldPointV27/);assert.match(source,/previousTip\.x,previousTip\.y/);
assert.match(source,/bladeTangentV27\(action,frame\)/);assert.match(source,/lerpPoint\(root,tip,\.88\)/);
...
assert.match(main,/GameSceneV33 -> GameSceneV32 -> GameSceneV31 -> GameSceneV30 -> GameSceneV29 -> GameSceneV28 -> GameSceneV27/);
assert.match(v33,/extends GameSceneV32/);assert.match(v32,/extends GameSceneV31/);
assert.match(v31,/extends GameSceneV30/);assert.match(v30,/extends GameSceneV29/);assert.match(v29,/extends GameSceneV28/);
console.log('V27 blade-locked VFX verification passed beneath V33.');
