...
const s=Object.create(GameSceneV19.prototype);
s.dead=false;
s.rollEndsAt=1200;
assert.equal(s.isRollInvulnerable(1100),true);
assert.equal(s.isRollInvulnerable(1200),false);
s.dead=true;
assert.equal(s.isRollInvulnerable(1100),false);

const route=Object.create(GameSceneV19.prototype);
assert.equal(route.isBranchDepth(0),true,'room 1 must return to the normal route branch');
assert.equal(route.isBranchDepth(1),false);
assert.equal(route.isBranchDepth(2),true);
assert.equal(route.isBranchDepth(3),false);
