...
// The reported bug was caused by the base combo capping at step 2. Normal
// presses inside the combo window must now cycle 0 -> 1 -> 2 -> 0 forever.
...
// Each sword strike owns a genuinely different motion path.
...
assert.match(source,/trail\.arc\(/,'V25 must render curved blade trails instead of only rectangular streaks');
assert.doesNotMatch(source,/super\.emitAttackMotionFx/,'V25 must fully replace the old generic sword VFX path');

const main=readFileSync('src/main.js','utf8');
assert.match(main,/import \{ GameSceneV25 \} from '\.\/GameSceneV25\.js'/,'main must import V25');
assert.match(main,/scene: \[GameSceneV25\]/,'main must boot V25');

console.log('Sword cycling and attack-specific V25 presentation verification passed.');
