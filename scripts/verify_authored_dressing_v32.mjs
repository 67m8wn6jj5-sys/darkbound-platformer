...
assert.match(source,/extends GameSceneV31/);assert.match(source,/ENVIRONMENT_ART_V30\.background\.key/);assert.match(source,/ENVIRONMENT_ART_V30\.architecture\.key/);
assert.match(source,/addAuthoredLightV32/);assert.match(source,/ENVIRONMENT_ART_V30\.backgroundObjects/);assert.match(source,/ENVIRONMENT_ART_V30\.arches/);
...
const main=readFileSync('src/main.js','utf8');const v33=readFileSync('src/GameSceneV33.js','utf8');
assert.match(main,/import \{ GameSceneV33 \} from '\.\/GameSceneV33\.js'/);assert.match(main,/scene: \[GameSceneV33\]/);assert.match(v33,/extends GameSceneV32/);
assert.doesNotMatch(v33,/dressAuthoredWorldV32\(|addContinuousBackgroundV32\(|renderArchitectureV32\(/,'V33 intentionally retires V32 background/arch composition');
console.log('Historical V32 asset/dressing verification passed; V33 correctly overrides its presentation.');
