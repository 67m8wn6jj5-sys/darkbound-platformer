...
function hashV37(seed,salt=0){
  let value=((Number(seed)||1)>>>0)^Math.imul((salt+1)>>>0,0x9e3779b1);
...
function floorAtV37(layout,x){
  return (layout?.floorSegments||[])
...
function safeFloorSpotV37(layout,section,preferredX){
  ...
  const x=Math.max(floor.x+80,Math.min(floor.x+floor.w-80,preferredX));
  return{x,y:floor.y,floor};
}
...
export class GameSceneV37 extends GameSceneV36 {
  ...
  createSoulHudV37(){
    if(this.v37SoulHud?.icon?.active)return;
    const depth=2100;
  ...
  layoutSoulHudV37(){
    ...
    this.v37SoulHud.halo.setPosition(28,y);
    this.v37SoulHud.icon.setPosition(28,y);
    this.v37SoulHud.value.setPosition(44,y-10);
  }

  updateSoulHudV37(){
    if(this.v37SoulHud?.value)this.v37SoulHud.value.setText(String(this.ensureRunStateV37().souls));
  }
  ...
  layoutRelicChoiceUIV37(){
    ...
    const cardW=Math.min(330,Math.max(230,w*.34));
    const cardH=Math.min(178,Math.max(140,h*.46));
    const y=Math.max(148,h*.58);
    const xs=[w*.30,w*.70];
  ...
  openRelicChoiceV37(altar){
    ...
    if(!choices.length){run.claimedAltars.add(altar.key);return;}
    this.v37ChoiceActive=true;
    this.v37ChoiceAltar=altar;
    this.v37ChoiceOptions=choices;
    this.v37ChoiceSelection=0;
    ...
    choices.forEach((choice,index)=>{
      const card=this.v37RelicUI.cards[index];
      card.name.setText(choice.name);card.type.setText('RELIC');card.desc.setText(choice.description);
    });
    this.setRelicChoiceVisibleV37(true);
    this.layoutRelicChoiceUIV37();
    this.refreshRelicSelectionV37();
  }

  chooseRelicV37(index){
    if(!this.v37ChoiceActive||(this.time?.now||0)<(this.v37ChoiceInputLockUntil||0))return;
    const altar=this.v37ChoiceAltar;
    const relic=this.v37ChoiceOptions[index];
    if(!altar||!relic)return;
    const run=this.ensureRunStateV37();
    if(run.souls<altar.cost)return;
    run.souls-=altar.cost;
    run.relics.add(relic.id);
    run.claimedAltars.add(altar.key);
    altar.claimed=true;
    altar.core?.setAlpha?.(.06);
    this.applyRelicV37(relic);
    this.v37ChoiceActive=false;
    this.v37ChoiceAltar=null;
    this.v37ChoiceOptions=[];
    this.setRelicChoiceVisibleV37(false);
    this.touchControls?.setVisible(true);
    if(!this.dead&&!this.pausePanel?.bg?.visible&&!this.routeActive)this.physics.resume();
    this.updateSoulHudV37();
    this.spawnGreenBurst?.(this.player.x,this.player.y-18,24,68,52,340);
  }
  ...
  handleRelicChoiceInputV37(){
    ...
    if(keyboard){
      ...
      if(Phaser.Input.Keyboard.JustDown(this._v37Keys.left)||Phaser.Input.Keyboard.JustDown(this._v37Keys.left2)){this.v37ChoiceSelection=(this.v37ChoiceSelection+1)%2;this.refreshRelicSelectionV37();}
      if(Phaser.Input.Keyboard.JustDown(this._v37Keys.right)||Phaser.Input.Keyboard.JustDown(this._v37Keys.right2)){this.v37ChoiceSelection=(this.v37ChoiceSelection+1)%2;this.refreshRelicSelectionV37();}
    ...
    if((left&&!this._v37Pad.left)||(right&&!this._v37Pad.right)){this.v37ChoiceSelection=(this.v37ChoiceSelection+1)%2;this.refreshRelicSelectionV37();}
  ...
  addAltarV37(layout,slot,sectionIndex){
    ...
    const preferredX=section.start+(slot===0?1120:430);
    const spot=safeFloorSpotV37(layout,section,preferredX);if(!spot)return;
    const stage=layout.stageIndex||0,key=`${stage}:altar:${slot}`,cost=SOUL_RELIC_V37.altarCosts[slot]||8;
    const claimed=this.ensureRunStateV37().claimedAltars.has(key);
    ...
    const altar={key,slot,cost,x:spot.x,y:spot.y,core,claimed};
    this.v37Altars.push(altar);
  }

  addChestV37(layout,slot,sectionIndex){
    const section=layout.sections?.[sectionIndex];if(!section)return;
    const candidates=(layout.platforms||[]).filter(spec=>spec.section===section.id&&spec.role==='upper').sort((a,b)=>a.y-b.y);
    const platform=candidates[0];if(!platform)return;
    const x=Math.round((platform.x+platform.w*.5)/32)*32,y=platform.y;
    const stage=layout.stageIndex||0,key=`${stage}:chest:${slot}`,opened=this.ensureRunStateV37().openedChests.has(key);
    ...
    this.v37Chests.push({key,slot,x,y,base,lid,latch,opened,value:slot===0?6:9});
  }
  ...
  markEliteV37(layout){
    if(!layout||this.v34BossMode)return;
    ...
    if(!enemy)return;
    enemy.v37Elite=true;
    enemy.maxHp=(enemy.maxHp||enemy.hp||TUNING.enemyMaxHp)+2;
    enemy.hp=(enemy.hp||TUNING.enemyMaxHp)+2;
    ...
    this.addV28Decor(aura);
    this.v37EliteAura={enemy,aura};
  }

  rebuildRoomLayout(template){
    this.ensureRunStateV37();
    this.v37Altars=[];this.v37Chests=[];this.v37EliteAura=null;
  ...
  spawnSoulDropV37(enemy,amount){
    ...
    const count=Math.max(1,Math.min(6,Math.ceil(amount/2)));
    let remaining=amount;
    for(let index=0;index<count;index++){
      const slotsLeft=count-index;
      const value=Math.max(1,Math.ceil(remaining/slotsLeft));remaining-=value;
      const x=enemy.sprite.x+(index-(count-1)*.5)*10,y=enemy.sprite.y-28;
      const halo=this.add.circle(0,0,8,SOUL_COLOR,.11).setBlendMode(Phaser.BlendModes.ADD);
      const core=this.add.circle(0,0,3.2,SOUL_HOT,.96);
      const sprite=this.add.container(x,y,[halo,core]).setDepth(95);
      const angle=-Math.PI*.82+(count===1?0:index/(count-1)*Math.PI*.64);
      const speed=145+index*13;
      this.v37SoulDrops.push({sprite,value,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed-105,age:0});
    }
  }

  collectSoulV37(drop){
    if(!drop?.sprite?.active)return;
    this.ensureRunStateV37().souls+=drop.value||1;
    drop.collected=true;
    drop.sprite.destroy();
    this.updateSoulHudV37();
  }
  ...
  surfaceBelowV37(x,y){
    const layout=this.environmentLayout;
    return [...(layout?.platforms||[]),...(layout?.floorSegments||[])]
      .filter(spec=>x>=spec.x+4&&x<=spec.x+spec.w-4&&spec.y>=y-10)
      .sort((a,b)=>a.y-b.y)[0]||null;
  }

  updateSoulDropsV37(delta){
    if(!this.v37SoulDrops?.length)return;
    const dt=Math.min(34,Math.max(0,Number(delta)||16))/1000;
    const px=this.player?.x||0,py=(this.player?.y||0)-18;
    for(const drop of this.v37SoulDrops){
      if(drop.collected||!drop.sprite?.active)continue;
      drop.age+=dt;
      const dx=px-drop.sprite.x,dy=py-drop.sprite.y,dist=Math.hypot(dx,dy);
      if(dist<SOUL_RELIC_V37.magnetRadius&&drop.age>.18){
        const strength=(1-dist/SOUL_RELIC_V37.magnetRadius)*1250+360;
        const inv=1/Math.max(1,dist);
        drop.vx+=dx*inv*strength*dt;drop.vy+=dy*inv*strength*dt;
      }else drop.vy+=SOUL_RELIC_V37.soulGravity*dt;
      drop.vx*=Math.pow(.987,dt*60);
      drop.sprite.x+=drop.vx*dt;drop.sprite.y+=drop.vy*dt;
      const surface=this.surfaceBelowV37(drop.sprite.x,drop.sprite.y);
      if(surface&&drop.vy>0&&drop.sprite.y>=surface.y-7){
        drop.sprite.y=surface.y-7;drop.vy*=-.34;drop.vx*=.74;
        if(Math.abs(drop.vy)<28)drop.vy=0;
      }
      if(dist<SOUL_RELIC_V37.collectRadius||drop.sprite.y>840)this.collectSoulV37(drop);
    }
  ...
  onEnemyKilledV37(enemy){
    ...
    if(this.hasRelicV37('soulLeech')&&run.kills%5===0){
      ...
      this.playerHp=Math.min(maxHp,(this.playerHp||0)+1);
      this.pixelArt?.setTintFill?.(0xbfffc9);
      this.time.delayedCall(100,()=>this.pixelArt?.clearTint?.());
    }
  }

  emitGreenFlameV37(origin){
    const facing=this.facing<0?-1:1;
    const x=origin?.sprite?.x??this.player.x,y=(origin?.sprite?.y??this.player.y)-24;
    ...
    const targets=(this.enemies||[]).filter(enemy=>enemy?.alive&&enemy!==origin&&Math.sign((enemy.sprite?.x||x)-x)===facing&&Math.abs((enemy.sprite?.x||x)-x)<=235&&Math.abs((enemy.sprite?.y||y)-y)<=120);
    this.v37WaveDamage=true;
    try{for(const target of targets.slice(0,2))this.damageEnemy(target,0);}finally{this.v37WaveDamage=false;}
  }
  ...
  damageEnemy(enemy,step){
    ...
    const baseDamage=this.runStats?.damage||0;
    ...
    if(!this.v37WaveDamage){
      ...
      if(this.hasRelicV37('relentless')){
        if(now-this.v37LastHitAt<=920)this.v37RapidHits=(this.v37RapidHits||0)+1;else this.v37RapidHits=1;
        this.v37LastHitAt=now;
        if(this.v37RapidHits>=3){temporaryBonus++;this.v37RapidHits=0;}
      }
    }
  ...
  startRoll(time,body){
    super.startRoll(time,body);
    if(this.hasRelicV37('phantomStep'))this.lastRollAt=time-180;
  }

  openChestV37(chest){
    if(!chest||chest.opened)return;
    const run=this.ensureRunStateV37();
    chest.opened=true;run.openedChests.add(chest.key);
    chest.lid?.setAngle?.(-18)?.setPosition?.(chest.x-4,chest.y-40);chest.latch?.setVisible?.(false);
    const fakeEnemy={sprite:{x:chest.x,y:chest.y-20}};
    this.spawnSoulDropV37(fakeEnemy,chest.value);
    this.spawnGreenBurst?.(chest.x,chest.y-24,12,42,30,210);
  }
  ...
  updateWorldRewardsV37(){
    ...
    for(const altar of this.v37Altars||[]){
      ...
      const affordable=run.souls>=altar.cost;
      altar.core?.setAlpha?.(affordable?.24:.07);
      if(affordable&&Math.hypot(px-altar.x,py-(altar.y-50))<86){this.openRelicChoiceV37(altar);return;}
    }
    for(const chest of this.v37Chests||[]){
      if(chest.opened||run.openedChests.has(chest.key))continue;
      if(Math.hypot(px-chest.x,py-(chest.y-24))<58)this.openChestV37(chest);
    }
  }
  ...
  // V37 retires the prototype post-room three-card stat screen. Relics now
  // exist physically inside the traversal space, so clearing a stage flows
  // directly into the route decision or next stage.
  openReward(roomIndex){
    if(this.v34BossMode)return super.openReward(roomIndex);
  ...
  update(time,delta){
    if(this.v37ChoiceActive){
      this.handleRelicChoiceInputV37();
      this.hidePrototypeTextV35();
      this.suppressPrototypeGeometryV35();
      return;
    }
    super.update(time,delta);
    this.updateSoulDropsV37(delta);
    this.updateWorldRewardsV37();
    this.updateEliteAuraV37();
    this.updateSoulHudV37();
  }
}
