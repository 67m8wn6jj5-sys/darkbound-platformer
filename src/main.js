...
function startGame() {
  ...
  const config = {
    // iOS orientation changes are substantially more reliable with the Canvas
    // renderer than with a newly-created/resized WebGL surface.
    ...
    height: 720,
    scale: {
      // Keep a stable logical game surface. Phaser only scales the presentation
      // to the current landscape viewport instead of rebuilding the renderer.
...
function handleViewportChange() {
  ...
  startTimer = setTimeout(() => {
    if (isPortraitPhone()) {
      // If a running session is rotated back to portrait, fully dispose of the
      // renderer. Returning to landscape will create a fresh, known-good canvas.
      if (game) {
        game.destroy(true);
        game = null;
