...
const CACHE_BUST='protagonist-v39-rescue-20260823-1';
...
function rescueFrameKey(action,direction,index){
  return `${RESCUE_PREFIX}${frameKey(action,direction,index)}`;
}
...
function rescueRotationKey(source,direction){
  return `${RESCUE_PREFIX}${rotationKey(source,direction)}`;
}
...
export class GameSceneV39 extends GameSceneV38 {
  preload(){
    ...
    // V39 intentionally loads a second copy of every protagonist texture under
    // fresh keys. This bypasses stale/failed iOS Safari cache entries without
    // disturbing the established animation manifests or combat state machine.
  ...
  repairProtagonistRenderV39(){
    ...
    this.player.setVisible?.(true).setDepth?.(90);

    const art=this.pixelArt;
    if(!art)return;

    // The inherited animation system keeps currentPixelKey pointed at the
    // canonical px-* key. Render the fresh V39 copy of that exact frame.
    ...
    art.setVisible(true).setDepth(300);
